...
class SSHService {
    ...
    try {
      ...
      if (connection.authMethod == AuthMethod.password) {
        if (password == null || password.isEmpty) {
          await socket.close();
          throw SSHException('Password is required for password authentication');
        }
        _client = SSHClient(
          socket,
      ...
      _isConnected = false;
      throw SSHException('Authentication failed: All authentication methods failed', e);
    } on SocketException catch (e) {
      _isConnected = false;
      if (e.message.contains('Connection refused')) {
        throw SSHException('Connection refused. Is SSH server running on ${connection.host}:${connection.port}?', e);
      } else if (e.message.contains('Connection timed out')) {
        throw SSHException('Connection timed out. Check if ${connection.host} is reachable.', e);
      ...
      if (errorStr.contains('HandshakeException')) {
        throw SSHException('SSH handshake failed. Server may not support SSH protocol.', e);
      }
  ...
  Future<SSHSession?> startSession({int cols = 80, int rows = 24}) async {
